...
| `server/db/migrate_quotes_status_simplify.js` | Migrates `quotes.status` to `draft\|sent\|accepted` only (pending→draft, review→sent, declined→sent); updates CHECK constraint | ✅ Applied |
| `server/db/migrate_qb_status_simplify.js` | Migrates `qb_quote_headers.status` to `draft\|sent\|accepted` only (submitted→sent, declined→sent, locked→accepted); updates CHECK constraint | ✅ Applied |
